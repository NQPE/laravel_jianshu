...
@section('content')
    <div class="col-sm-8 blog-main">
        <form  id="submitPost" action="/posts/update/{{$post->id}}" method="POST">
            {{csrf_field()}}
            <div class="form-group">
                <label>标题</label>
                <input name="title" type="text" class="form-control" placeholder="这里是标题" value="{{old('title')?:$post->title}}">
            </div>
            <div class="form-group">
                <label>内容</label>
    ...
    <script type="text/javascript">
        ...
        editor.create();
        {{--console.log(HTMLDecode("{{ $post->content }}"));--}}
        {{--editor.txt.html(HTMLDecode("{{ $post->content }}"));--}}
        editor.txt.html(HTMLDecode("{{old('content')?:$post->content }}"));
        {{--@if(old('content'))--}}
        {{--alert({{ old('content') }});--}}
        {{----}}
        {{--@endif--}}
